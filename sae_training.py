...
def train_epoch(
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        optimizer.zero_grad()
        reconstructed, h, h_sparse, reconstructed_1, residual_1, reconstructed_2 = model.module.forward_1d_normalized(
            batch_normalized
        )

        # Compute main loss in normalized space
        loss = criterion(reconstructed, batch_normalized)
        loss_step1 = criterion(reconstructed_1.detach(), batch_normalized)
        loss_step2 = criterion(reconstructed_2.detach(), residual_1.detach())
        residual_norm = torch.linalg.norm(residual_1.detach(), dim=-1).mean()
        ...
        if dead_latents >= k_aux:
            ...
            residual = batch_normalized - reconstructed_1.detach()
...
def validate_epoch(
    ...
    dist.all_reduce(loss_step2_acc, op=dist.ReduceOp.SUM)
    dist.all_reduce(residual_norm_acc, op=dist.ReduceOp.SUM)
    world_size_len_dataloader = dist.get_world_size() * len(dataloader)
    avg_loss = loss_acc.item() / world_size_len_dataloader
    avg_aux_loss = aux_loss_acc.item() / world_size_len_dataloader
    avg_total_loss = total_loss_acc.item() / world_size_len_dataloader
    avg_loss_step1 = loss_step1_acc.item() / world_size_len_dataloader
    avg_loss_step2 = loss_step2_acc.item() / world_size_len_dataloader
    avg_residual_norm = residual_norm_acc.item() / world_size_len_dataloader

    return avg_loss, avg_aux_loss, avg_total_loss, avg_loss_step1, avg_loss_step2, avg_residual_norm
...
def train_autoencoder(
    ...
    for epoch in range(num_epochs):
        ...
        # Validate an epoch
        val_avg_loss, val_avg_aux_loss, val_avg_total_loss, val_avg_loss_step1, val_avg_loss_step2, val_avg_residual_norm = validate_epoch(
            epoch=epoch,
            num_epochs=num_epochs,
            model=model,
