...
def build_run_name(run_name: str | None, dict_reg_coeff: float, vmf_kappa: float) -> str:
    ...
    return f"dense-kernel-low-pressure_coeff-{coeff_token}_kappa-{kappa_token}_{timestamp}"
...
def main() -> None:
    ...
    run_name = build_run_name(args.run_name, dict_reg_coeff, vmf_kappa)
    ...
    if rank == 0:
        ...
        wandb.init(
            ...
            name=run_name,
            config={
                "experiment_variant": "dense-kernel-low-pressure",
                "d_model": d_model,
                "n_latents": n_latents,
                "k": k,
                "k_aux": k_aux,
