#!/usr/bin/env python3
"""Train an exactly matched ReLU and sample-nested shared-dictionary SAE."""
...
class ReLUFromScratchSAE(nn.Module):
    ...
    def module_parameter_delta(
        ...
        if initial:
            raise ValueError("ReLU control has no nested-module snapshot")
        return 0.0
...
class SampleNestedReLUSAE(ReLUFromScratchSAE):
    ...
    def __init__(
        ...
        inner_features: int,
        seed: int,
        turn_loss_weight: float,
        inner_l1_weight: float,
    ) -> None:
        if inner_features <= 0 or inner_features >= n_latents:
            raise ValueError("inner_features must lie strictly inside the dictionary")
        if turn_loss_weight <= 0:
            raise ValueError("turn_loss_weight must be positive")
        if inner_l1_weight < 0:
            raise ValueError("inner_l1_weight must be non-negative")
        super().__init__(b_pre=b_pre, n_latents=n_latents, seed=seed)
        self.inner_features = int(inner_features)
        self.turn_loss_weight = float(turn_loss_weight)
        self.inner_l1_weight = float(inner_l1_weight)
    ...
    def l1_per_token(
        ...
        return token_l1 + self.inner_l1_weight * inner_l1
    ...
    def reconstruction_losses(
        ...
        return {
            "total": token + self.turn_loss_weight * inner,
            "token": token,
            "semantic": inner,
        }
...
def evaluate_validation(
    model: ReLUFromScratchSAE | SampleNestedReLUSAE,
...
def main() -> None:
    ...
    parser.add_argument("--turn-loss-weight", type=float, default=1.0)
    parser.add_argument("--inner-l1-weight", type=float, default=1.0)
    ...
    set_seed(args.seed)
    candidate = SampleNestedReLUSAE(
        b_pre=b_pre,
        n_latents=args.n_total,
        inner_features=args.inner_features,
        seed=args.initialization_seed,
        turn_loss_weight=args.turn_loss_weight,
        inner_l1_weight=args.inner_l1_weight,
    )
    candidate_result = train_one(
        "structured sample-nested shared-dictionary candidate",
        candidate,
        cache,
        args,
        device,
    ...
    candidate_path = (
        args.output_dir
        / "trained_sae-structured-sample-nested-relu.pt"
    )
    ...
    targets = [
        ...
        {
            "label": "structured-cache sample-nested ReLU SAE",
            "kind": "relu",
            "layer": args.layer,
    ...
    targets_path = args.output_dir / "targets-structured-sample-nested.json"
    targets_path.write_text(
        json.dumps(targets, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
    summary = {
        "experiment": "parameter-matched structured-cache sample-nested SAE",
        "arguments": {
    ...
    summary_path = args.output_dir / "train-summary-structured-sample-nested.json"
