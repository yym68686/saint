#!/usr/bin/env python3
"""Measure label-free health and drift of BatchTopK dictionary partitions."""
...
from train_structured_nested_batchtopk_sae import (
...
def activation_stats(
    ...
    if missing:
        raise KeyError(f"Missing BatchTopK checkpoint keys: {sorted(missing)}")
    width = int(state["encoder.weight"].shape[0])
    threshold = float(state["threshold"].float().item())
    if threshold <= 0:
        raise ValueError(f"BatchTopK threshold must be positive, got {threshold}")
    groups = {
    ...
    group_results = {}
    total_nonzero = int(active_counts.sum().item())
    for name, (start, end) in groups.items():
        ...
        group_results[name] = {
            ...
            "active_event_fraction": (
                nonzero_count / total_nonzero if total_nonzero else 0.0
            ),
    ...
    result["mean_total_l0_per_token"] = total_nonzero / tokens
...
def direction_drift(
    ...
    result["threshold_delta"] = float(
        candidate["threshold"].float().item()
        - base["threshold"].float().item()
    )
    return result
