#!/usr/bin/env python3
"""Apply the preregistered Initial3 gate to CP multi-view SAE v1."""
...
def main() -> None:
    ...
    rows = {row["variant_key"]: row for row in payload["architecture_results"]}
    summary = {row["variant_key"]: row for row in payload["summary"]}
    required = {"candidate", "mean_pooled_control", "wrong_alignment", "frozen_relu"}
    missing = sorted(required - set(rows))
    if missing:
        raise KeyError(f"Missing eval variants: {missing}")
    controls = ("mean_pooled_control", "wrong_alignment", "frozen_relu")
    ...
    gates = {
        ...
        "candidate_within_0p01_of_same_dataset_reference": candidate_mean
        >= args.reference_initial3 - args.maximum_reference_gap,
    }
    passed = all(gates.values())
    report = {
        "experiment": "CP factorized multi-view SAE v1 Initial3 gate",
    ...
    lines = [
        "# CP multi-view v1 Initial3 gate",
        "",
        ...
        f"- Gap vs same-dataset reference: `{candidate_mean - args.reference_initial3:+.6f}`",
