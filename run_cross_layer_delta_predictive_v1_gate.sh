...
CODE="${CODE:-/autodl-fs/data/worktrees/saint-cross-layer-delta-predictive-v1}"
...
ROOT="${ROOT:-/autodl-fs/data/cross_layer_delta_predictive_v1_gate_20260711}"
SIGNAL_TOKENS="${SIGNAL_TOKENS:-8192}"
...
if branch != "scpg-cross-layer-delta-predictive-v1":
...
payload = {
    "experiment": "cross-layer next-delta predictive frozen signal gate",
    ...
    "signal": {
        ...
        "definition": "diagonally whitened multivariate correlation norm between each L22 ReLU feature and the aligned normalized L23-L22 residual delta",
        "weight": "rank map to [0.5,1.5] with mean 1.0",
        "rank_tie_break": "independent 1e-7 random jitter from pre-registered seeds",
        "evaluation_uses_only_layer22_features": True,
    },
    "controls": {
        "raw_l22_relu": True,
        "feature_permuted_same_weight_distribution": True,
        "wrong_token_complete_delta_derangement": True,
        "permutation_seed": 43026,
    },
    "evaluation": {
        "datasets": [
...
"$PY" "$CODE/diagnose_cross_layer_delta_predictive.py" \
  --checkpoint "$CHECKPOINT" \
  --cache-dir "$CACHE_DIR" \
  --model-dir "$MODEL_DIR" \
  --output-dir "$ROOT" \
  --signal-tokens "$SIGNAL_TOKENS" \
  --signal-batch-tokens 128 \
  --score-feature-block 2048 \
  --permutation-seed 43026 \
  --datasets \
    LabHC/bias_in_bios_class_set3 \
    canrager/amazon_reviews_mcauley_1and5 \
    fancyzhx/ag_news \
...
from pathlib import Path
payload = json.loads(
    Path("$ROOT/cross-layer-delta-predictive-gate.json").read_text()
)
report = {
