...
CODE="${CODE:-/autodl-fs/data/worktrees/saint-structured-sample-energy-v2}"
...
ROOT="${ROOT:-/autodl-fs/data/structured_sample_energy_v2_gate_20260711}"
...
if branch != "scpg-structured-sample-energy-v2":
...
payload = {
    "experiment": "structured mean-preserving sample-dispersion frozen gate",
    ...
    "mechanism": {
        ...
        "candidate": "mean_t z_t + sqrt(max(mean_t z_t^2 - mean_t(z_t)^2, 0))",
        "candidate_coefficient": 1.0,
        "component_control": "standard deviation without the mean path",
        "prior_best_control": "sqrt(mean_t z_t^2)",
        "causal_control": "row-balanced wrong-sample mean-plus-std with zero same-class pairs and full row-multiset preservation",
    ...
    "gate": {
        ...
        "official_candidate_minus_prior_rms": ">= -0.003",
        "candidate_minus_wrong_sample": ">= 0.05",
    },
    "leakage": {
...
"$PY" "$CODE/test_structured_sample_energy_v2.py" \
  | tee "$ROOT/mechanism-tests.log"

"$PY" "$CODE/diagnose_structured_sample_energy_v2.py" \
...
payload = json.loads(Path("$ROOT/sample-energy-v2-gate.json").read_text())
report = {
