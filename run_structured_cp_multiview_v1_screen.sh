...
cp structured_cp_multiview_v1_preregistration.json "$ROOT/preregistration.json"
...
"$PYTHON_BIN" test_structured_cp_multiview_sae.py \
...
CUDA_VISIBLE_DEVICES=0 "$PYTHON_BIN" train_structured_cp_multiview_sae.py \
  --cache-dir "$CACHE_DIR" \
...
"$PYTHON_BIN" validate_cp_multiview_training.py \
  --summary "$ROOT/train/train-summary-cp-multiview-v1.json" \
  --output-json "$ROOT/training-integrity.json" \
  | tee "$ROOT/training-integrity.log"

CUDA_VISIBLE_DEVICES=0 "$PYTHON_BIN" saebench_sparse_probing_cp_multiview.py \
  --eval-script "$EVAL_SCRIPT" \
  --targets-json "$ROOT/train/targets-cp-multiview-v1.json" \
...
"$PYTHON_BIN" analyze_cp_multiview_gate.py \
...
if report["pass"]:
    print("The short screen passed. Long training may be scheduled; full7 remains prohibited before long training.")
else:
