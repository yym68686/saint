#!/usr/bin/env python3
"""Apply the preregistered Initial3 gate to Cascaded Concept SAE v3."""
...
def main() -> None:
    ...
    required = {
        "v396_finetune",
        "cascaded_concept_v3",
        "cascaded_concept_v3_wrong_hierarchy",
        "cascaded_concept_v3_level1_only",
    }
    missing = sorted(required - set(rows))
    if missing:
        raise KeyError(f"Evaluation is missing {missing}")

    candidate = rows["cascaded_concept_v3"]["mean_acc"]
    ...
    dataset_deltas = {}
    for dataset_name, metrics in details["cascaded_concept_v3"][
        "dataset_results"
    ].items():
    ...
    candidate_train = train["results"]["cascaded_concept_v3"]
    report: dict[str, Any] = {
    ...
    lines = [
        "# Cascaded Concept SAE v3 Initial3 Gate",
        "",
        "| Variant | Mean Acc |",
        "|---|---:|",
    ]
