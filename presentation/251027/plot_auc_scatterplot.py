...
def plot_auc_scatterplot():
    """
    Parses feature activation summaries for dense and batchtopk models across multiple features,
    and generates a scatter plot to compare their AUC-ROC scores for each feature.
    """
    ...
    logging.info("Processing features for AUC-ROC scatter plot...")
    ...
    for feature_key, feature_name in features.items():
        ...
        try:
            ...
            with open(dense_json_path, 'r') as f:
                dense_auc = json.load(f)["auc_roc"]["max"]

            with open(batchtopk_json_path, 'r') as f:
                batchtopk_auc = json.load(f)["auc_roc"]["max"]

            plot_data.append({
                "name": feature_name,
                "batchtopk": batchtopk_auc,
                "dense": dense_auc
            })
            logging.info(f"  - Data for '{feature_name}': BatchTopK={batchtopk_auc:.4f}, Dense={dense_auc:.4f}")
    ...
    for point in plot_data:
        ax.text(point['batchtopk'], point['dense'] + 0.0002, point['name'], ha='center', va='bottom', fontsize=9)
    ...
    # Set a more reasonable start for AUC, e.g., 0.95, if all data points are high
    lims[0] = max(0.95, lims[0])
    ax.plot(lims, lims, 'k--', alpha=0.75, zorder=1, label='y = x (Perfect Agreement)')

    ax.set_xlabel("batchtopk (SOTA) AUC-ROC", fontsize=14)
    ax.set_ylabel("sae_exp11_dense (Ours) AUC-ROC", fontsize=14)
    ax.set_title("AUC-ROC Consistency Across 5 Features", fontsize=16, pad=20)
    ...
    output_path = output_dir / "auc_consistency_scatterplot.png"
    plt.savefig(output_path, dpi=300)
    logging.info(f"\nPlot saved to {output_path}")

if __name__ == "__main__":
    plot_auc_scatterplot()
